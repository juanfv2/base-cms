...
export class CountryAutoCompleteComponent extends BaseCmsAutoCompleteComponent implements ControlValueAccessor {
  ...
  override searchTerm(term: string): Observable<any> {
    ...
    mEvent.select = [this.labels.country.id.field, this.labels.country.name.field, this.labels.country.code.field]
    mEvent.sorts = [new JfSort(`${this.labels.country.id.field}`, JfSort.asc)]
